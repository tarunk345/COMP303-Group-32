        ...
        if armor is None:
            ...
            if(isinstance(armor,(Armor,Potion))): 
                ...
                if defense_changed>=0 and attack_changed>=0:
                    change_text = self.get_image_name() + 'changed! Defense' +'increased by' + str(defense_changed)+'\n' + 'attack increased by' + str(attack_changed)+'!'
                elif defense_changed<0: 
                    change_text = self.get_image_name() + 'changed! Defense' +'decreased by' + str(abs(defense_changed))+'\n' + 'attack increased by' + str(attack_changed)+'!'
...
class Attack_potion(Potion):
    ...
    def get_attack_value(self) -> int:
        return self.get_multiplier() * self.get_attack_value()
    ...
    def player_entered(self, player: "HumanPlayer") -> list[Message]:
        ...
        pick_text = 'You picked up the '+ self.get_image_name() +'! It is applied to ' + str(self.__armor)+ '.\nAttack increased by ' + str(attack_increase)+'!'
        self.set_fields() 
        return [DialogueMessage(self, player, pick_text, image_name)]
